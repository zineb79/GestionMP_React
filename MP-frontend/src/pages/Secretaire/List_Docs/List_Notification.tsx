...
const List_Notification = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const [notificationsData, marchesData] = await Promise.all([
                    getNotifications(),
                    getMarches()
                ]);
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                try {
                    ...
                    setDataSource(prev => prev.filter(not => not.id_NOTIF !== record.id_NOTIF));
    ...
    const columns = [
        {
            ...
            key: 'marche_NOTIF',
            render: (record: Notification) => record.marche_NOTIF_obj?.numOrdre || 'N/A',
                ...
                >
                    ...
                    >
                        ...
                        <Form.Item
                            name="dateVisa_NOTIF"
                            label="Date de visa"
                            rules={[{ required: true, message: 'Veuillez sélectionner la date de visa' }]}
                        >
                            <DatePicker 
                                style={{ width: '100%' }} 
                                format="DD/MM/YYYY"
                        ...
                        <Form.Item
                            ...
                            rules={[{ required: true, message: 'Veuillez sélectionner la date d\'approbation' }]}
                        >
                            <DatePicker 
                                style={{ width: '100%' }} 
                                format="DD/MM/YYYY"
