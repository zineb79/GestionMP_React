...
const List_AO = () => {
  ...
  const onDeleteAppelOffre = async (record: AppelOffre) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`/appelsoffre/${record.num_Ordre_AO}`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          });
          setDataSource((pre) => pre.filter((ao) => ao.num_Ordre_AO !== record.num_Ordre_AO));
        } catch (error) {
          console.error("Erreur lors de la suppression de l'appel d'offre :", error);
          alert("Impossible de supprimer l'appel d'offre. Veuillez réessayer plus tard.");
        }
      },
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(`/appelsoffre/${editingAppelOffre.num_Ordre_AO}`, editingAppelOffre, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      setDataSource((pre) =>
        pre.map((ao) =>
          ao.num_Ordre_AO === editingAppelOffre.num_Ordre_AO ? editingAppelOffre : ao
        )
      );
      resetEditing();
    } catch (error) {
      console.error("Erreur lors de la mise à jour de l'appel d'offre :", error);
      alert("Impossible de mettre à jour l'appel d'offre. Veuillez réessayer plus tard.");
    }
  };
      ...
      <div className="form">
          ...
          >
            ...
            <Input
              ...
              onChange={(e) =>
                setEditingAppelOffre((pre) =>
                  pre ? { ...pre, coutEstime_AO: parseFloat(e.target.value) } : pre
                )
              }
            />
            <Input
              ...
              onChange={(e) =>
                setEditingAppelOffre((pre) =>
                  pre ? { ...pre, cautionProvisoire_AO: parseFloat(e.target.value) } : pre
                )
              }
            />
            <Input
